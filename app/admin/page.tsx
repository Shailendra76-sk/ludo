...
export default function AdminPage(){
 const [stats,setStats]=useState<Stats|null>(null); const [users,setUsers]=useState<any[]>([]); const [games,setGames]=useState<any[]>([]); const [reports,setReports]=useState<any[]>([]); const [error,setError]=useState("");
 async function load(){
  const calls=["/api/admin/dashboard","/api/admin/users","/api/admin/games","/api/admin/reports"];
  ...
  setStats(data[0]);setUsers(data[1].users??[]);setGames(data[2].games??[]);setReports(data[3].reports??[]);
 }
