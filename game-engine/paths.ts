...
/**
 * Standard 15×15 Ludo board coordinates.
 * Source references express points as [row, column]; this web app uses
 * {x: column, y: row} to match CSS grid coordinates.
 *
 * Each player route contains 57 positions:
 * 52 shared-track cells followed by 5 private home-lane cells.
 * Step 58 is the final centre/home position.
 */

// Top-left RED route. This is the same geometry as the reference
// implementation's greenPath, with row/column converted to x/y.
...
// Top-right GREEN route.
const GREEN_ROUTE: BoardPoint[] = [
...
// Bottom-right YELLOW route.
const YELLOW_ROUTE: BoardPoint[] = [
...
// Bottom-left BLUE route.
const BLUE_ROUTE: BoardPoint[] = [
  { x: 6, y: 13 }, { x: 6, y: 12 }, { x: 6, y: 11 }, { x: 6, y: 10 }, { x: 6, y: 9 },
  { x: 5, y: 8 }, { x: 4, y: 8 }, { x: 3, y: 8 }, { x: 2, y: 8 }, { x: 1, y: 8 }, { x: 0, y: 8 },
  { x: 0, y: 7 }, { x: 0, y: 6 }, { x: 1, y: 6 }, { x: 2, y: 6 }, { x: 3, y: 6 }, { x: 4, y: 6 }, { x: 5, y: 6 },
  { x: 6, y: 5 }, { x: 6, y: 4 }, { x: 6, y: 3 }, { x: 6, y: 2 }, { x: 6, y: 1 }, { x: 6, y: 0 },
...
export const SHARED_TRACK = RED_ROUTE.slice(0, 52);

export const HOME_LANES = {
  red: RED_ROUTE.slice(52),
  green: GREEN_ROUTE.slice(52),
  yellow: YELLOW_ROUTE.slice(52),
  blue: BLUE_ROUTE.slice(52),
} as const;
...
// Exact star/safe cells from the standard board, converted from [row,column].
